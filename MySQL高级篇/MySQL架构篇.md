...
![image-20230125123443244](MySQL架构篇.assets/image-20230125123443244.png)

1）红色标签是执行步骤。第九是将查询到的结果缓存起来，第十是将结果返回。

2）查询缓存在mysql8.0就**废弃**了。
...
![image-20230125151558227](MySQL架构篇.assets/image-20230125151558227.png)

ps：如上图，先查询了profiling的值，然后选择了数据库，接着查看了数据库的表......
...
**注**：这样查询的是**最近一步**操作的执行步骤，等价于指定上面id为7的步骤。
...
查询 SQL 的执行时间结果和上面是一样的，因为第七步由上上图可知为查询employees表数据。
...
**support**表示是否支持该存储引擎；**Transactions**表示是否支持事务。

**XA**表示是否支持分布式事务；**Savepoints**即保存点，用于支持部分事务的回滚，其他引擎不支持事务所以它们都是NO。
...
如果在创建表的语句中没有显式指定表的存储引擎的话，那就会默认使用 InnoDB 作为表的存储引擎（mysql5.5之前是MyISAM）。 如果我们想改变表的默认存储引擎的话，可以这样写启动服务器的命令行：
...
![image-20230125221037570](MySQL架构篇.assets/image-20230125221037570.png)
...
![image-20230125220922759](MySQL架构篇.assets/image-20230125220922759.png)

### 4.5 CSV 引擎：存储数据时，以逗号分隔各个数据项

![image-20230125221247263](MySQL架构篇.assets/image-20230125221247263.png)
...
**注**：每个字段必须为**not null**类型，否则会报错：

![image-20230125221445834](MySQL架构篇.assets/image-20230125221445834.png)
...
![image-20230125223107601](MySQL架构篇.assets/image-20230125223107601.png)
